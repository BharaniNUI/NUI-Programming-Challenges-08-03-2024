let studentObj = [{ "id": 1, "first_name": "Darby", "last_name": "Headrick", "email": "dheadrick0@discovery.com", "state": "AZ", "science": 42, "maths": 12, "english": 12, "social": 65 },
...
{ "id": 19, "first_name": "maximumAverageMarky", "last_name": "Caroll", "email": "mcarolli@cisco.com", "state": "MD", "science": 60, "maths": 62, "english": 40, "social": 93 },
...
{ "id": 50, "first_name": "maximumAverageMarkimilianus", "last_name": "Caroline", "email": "mcaroline1d@discovery.com", "state": "FL", "science": 88, "maths": 7, "english": 2, "social": 71 },
...
// 1. Student with Best Average
function findBestAverage(inputObject) {
    if (typeof inputObject !== 'object' || !inputObject?.length > 0) {
        return `Invalid Input`;
    }

    let averageMark
    let maximumAverageMark = 0;
    let studentName = "";

    for (let i = 0; i < inputObject.length; i++) {
        const { science, maths, english, social, first_name, last_name } = inputObject[i]; //Destructuring inputObject
        averageMark = (science + maths + english + social) / 4; //Finding the average mark
        if (averageMark > maximumAverageMark) { //Comparing to get best average
            maximumAverageMark = averageMark;
...
console.log(findBestAverage(studentObj));

// 2. Student with Least Average
function findLeastAverage(inputObject) {
    if (typeof inputObject !== 'object' || !inputObject?.length > 0) {
        return `Invalid Input`;
    }

    let averageMark;
    let minimumAverageMark = 400;
    let studentName = "";

    for (let i = 0; i < inputObject.length; i++) {
        const { science, maths, english, social, first_name, last_name } = inputObject[i];
        averageMark = (science + maths + english + social) / 4; //Finding the average mark
        if (averageMark < minimumAverageMark) { //Comparing to get least average
            minimumAverageMark = averageMark;
            studentName = first_name + ' ' + last_name;
        }
    }
    return studentName;
...
console.log(findLeastAverage(studentObj));

// 3. State with Best Average
function findStateWithBestAvg(inputObject) {
    if (typeof inputObject !== 'object' || !inputObject?.length > 0) {
        return `Invalid Input`;
    }
    let resultObject = {};

    for (let student of inputObject) {

        let average = (student.maths + student.science + student.english + student.social) / 4;

        if (!resultObject.hasOwnProperty(student.state)) {
            resultObject[student.state] = [average];
        } else {
            resultObject[student.state].push(average);
        }
    }
    ...
    for (let state in resultObject) {
        let eachStateSum = resultObject[state].reduce((a, b) => {
            return a + b;
        }, 0)

        let allAverage = eachStateSum / resultObject[state].length; //Finding average from sum

        if (bestAverage < allAverage) {
            bestAverage = allAverage;
            bestState = state;
        }
    }
    return bestState;
...
console.log(findStateWithBestAvg(studentObj));

// 4. Paginated Results
function getPaginatedResults(data, page, limit) {
    if (typeof data !== 'object' || typeof page !== 'number' || typeof limit !== 'number'
        || page < 0 || limit < 0) {
        return `Invalid Input`;
    }
    ...
    const startValue = pageLimit - limit;

    for (let i = startValue; i < pageLimit; i++) {
...
console.log(getPaginatedResults(studentObj, 3, 8))
